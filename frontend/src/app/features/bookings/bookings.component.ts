          ...
          @if (!loading && !error) {
            @if (bookings.length === 0) {
              <div class="empty-state">
                <p class="empty-label">No bookings yet</p>
                <a routerLink="/catalog" class="browse-link">Browse Equipment →</a>
              </div>
            } @else {
              <table class="bookings-table">
                <thead>
                  <tr>
                    ...
                    <th class="th th-hide-md">Period</th>
                    <th class="th th-hide-sm">Total</th>
                ...
                <tbody>
                  @for (booking of bookings; track booking.id; let i = $index) {
                    <tr class="tr">
                      ...
                      <td class="td">
                        <div class="equipment-cell">
                          @if (booking.equipment_image) {
                            <img [src]="booking.equipment_image" [alt]="booking.equipment_name" class="equipment-thumb" />
                          } @else {
                            <div class="equipment-thumb-empty"></div>
                          }
                          <span class="equipment-name">{{ booking.equipment_name }}</span>
                        </div>
                      </td>
                      <td class="td td-hide-md">
                        <span class="period">
                          {{ booking.start_date }}
                      ...
                      <td class="td td-hide-sm">
                        <span class="total-price">₸{{ booking.total_price | number:'1.0-0' }}</span>
                      </td>
                      <td class="td">
                        <span [class]="statusClass(booking.status)">{{ booking.status | uppercase }}</span>
                      </td>
